...
@pytest.mark.parametrize(
    "product_names",
    load_json_file("test_data/product_comparison.json")["three_products"],
)
def test_validate_product_comparison_page_with_three_products(page, product_names):
    """TC_PC_017 — Validate the Product Comparison page when three products are added."""
    product_name_1, product_name_2, product_name_3 = product_names
    home_page = HomePage(page)
    search_results_page = SearchResultsPage(page)
    comparison_page = ProductComparisonPage(page)

    products = [product_name_1, product_name_2, product_name_3]

    # ===== Steps 1-3 repeated three times: Search and add products to comparison =====
    ...
    # ===== Step 5: Validate Product Comparison page with three products (ER-1) =====
    ...
    # Verify three price cells are displayed — one per product column
    expect(comparison_page.get_all_price_cells_in_table()).to_have_count(3)

    # Verify three 'Add to Cart' buttons are displayed
    expect(comparison_page.get_add_to_cart_button_in_table()).to_have_count(3)

    # Verify three 'Remove' links are displayed
    expect(comparison_page.get_remove_link_in_table()).to_have_count(3)
