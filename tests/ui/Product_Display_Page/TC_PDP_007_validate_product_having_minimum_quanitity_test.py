...
def test_validate_product_having_minimum_quantity_set(authenticated_page):
    ...
    assert actual_default_qty == TestData.MINIMUM_PRODUCT_QUANTITY, (
    ...
    assert TestData.MINIMUM_PRODUCT_QUANTITY in minimum_qty_info, (
    ...
    assert TestData.MINIMUM_PRODUCT_QUANTITY in feedback_text, (
        messages.PDP_MIN_QTY_WARNING_MISSING_QTY.format(
            qty=TestData.MINIMUM_PRODUCT_QUANTITY, actual=feedback_text
        )
    )
    assert messages.PDP_MIN_QTY_KEYWORD in feedback_text.lower(), (
    ...
    expect(shopping_cart_page.lbl_empty_cart_msg).to_contain_text(messages.CART_EMPTY_TEXT)
    ...
    assert final_feedback_text != TestData.EMPTY_VALUE, messages.PDP_ADD_TO_CART_FEEDBACK_EMPTY
    assert (
